...
//fills target hexagon with a low-opacity yellow-ish color
function highlightFill(t, context, intention) {

    let tile = getTile(t, hexmap);
...
function clickedOnBoard(event){
    ...
    //assembling the tiles, whose distances from the click location is less than 45 pixels
    for(let row of hexmap.contents){
        for(let tile of row){
    ...
    //from the assembled tiles, we find the one closest to the click location
    ...
    //from here, the variable "current" holds the tile object that matched the search
    highlightBorder(current, contexts.stageContext);
    highlightFill(current, contexts.stageContext, "select");

    //todo: fix the modes after you get the click events to work
    /*
    let modes = {
        1: mode1,
        2: mode2,
...
function mode1(selected_tile) {
    ...
    let neighbours = getNeighboursDistanceN(selected_tile.getVector(), panel.selectedUnit.movementRange, hexmap);
...
function mode3(selected_tile) {
    let neighbours = getNeighboursDistanceN(panel.selectedTile.getVector(), panel.selectedUnit.attackRange, hexmap);
    if(vectorIn(neighbours, selected_tile.getVector())){
        if((selected_tile !== panel.selectedTile) && selected_tile.unit){
            selected_tile.unit.hp -= panel.selectedUnit.attack;
            mode = 1;
            neighbours = getNeighboursDistanceN(panel.selectedTile, panel.selectedUnit.movementRange, hexmap);
            console.log(neighbours);
            panel.selectNeighbours(vectorsToTiles(neighbours));
        }
    }
...
function generateGameLocations (map) {

    let center = [new Vector(Math.round(this.width / 2), Math.round(this.height / 2))];
    let n = Math.round(Math.random() * 3);
    for (let i = 0; i < n; i++)
        center = expand(center, map);

    alert("reached line 386, 'expand' worked.");

    let a = Math.floor(Math.random() * center.length);
    let tile_cache = center[a];

    let shores = [];
    for (let v of map.contents)
        if (v.type === "SHORELINE")
            shores.append(new Vector(v.x, v.y));

    alert("reached line 397, shores gathered")

    let fairness = 0; //*
    do {
        let a = Math.floor(Math.random() * shores.length);
        let b = Math.floor(Math.random() * shores.length);
        while (b === a) {
            let a = Math.floor(Math.random() * shores.length);
            let b = Math.floor(Math.random() * shores.length);
        }
        var spawn1 = shores[a];
        var spawn2 = shores[b];
        let dD = Math.abs(spawn1.distanceFrom(tile_cache) - spawn2.distanceFrom(tile_cache));
        fairness += 1/dD;
        fairness += 1*(getTile(spawn1, map).type === getTile(spawn2, map).type);
        alert("fairness value : " + fairness);
    } while (fairness < 2.5);
    return [tile_cache, spawn1, spawn2];
}
